...
func createCourseCategories(db *gorm.DB) error {
	categories := []po2.CourseCategoryPO{
		{
			CourseID: 1,
			Category: "通识",
		},
...
func createTrainingPlan(db *gorm.DB) error {
	trainingPlans := []po2.TrainingPlanPO{
		{
			ID:         1,
			Degree:     "本科",
			Department: "SEIEE",
	...
	trainingPlanCourses := []po2.TrainingPlanCoursePO{
		{
			TrainingPlanID: 1,
			BaseCourseID:   1,
		},
