...
func GetUserListHandler(c *gin.Context) {
	...
	response := dto2.UserListResponse{
		Page:     request.Page,
...
func GetUserActivityHandler(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, dto2.BaseResponse{Message: "非法用户ID"})
		return
	}
	...
	if user.ID != userID {
		c.JSON(http.StatusForbidden, dto2.BaseResponse{Message: "无权查看他人信息！"})
		return
	}

	userSummary, err := service.GetUserActivityByID(c, user.ID)
	if err != nil {
		c.JSON(http.StatusNotFound, dto2.BaseResponse{Message: "此用户不存在！"})
		return
	}
...
func UpdateUserProfileHandler(c *gin.Context) {
	user := middleware.GetCurrentUser(c)
	if user == nil {
		c.JSON(http.StatusNotFound, dto2.BaseResponse{Message: "用户未登录！"})
		return
	}

	var request dto2.UserProfileDTO
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, dto2.BaseResponse{Message: "用户信息更新失败。"})
		return
	}
	c.JSON(http.StatusOK, dto2.BaseResponse{Message: "用户信息更新成功。"})
}
