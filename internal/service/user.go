package service

import (
	"context"

	"jcourse_go/internal/domain/user"
	"jcourse_go/internal/infrastructure/repository"
	"jcourse_go/internal/interface/dto"
	"jcourse_go/internal/model/converter"
	"jcourse_go/pkg/util"
)

func GetUserActivityByID(ctx context.Context, userID int64) (*user.UserActivity, error) {
	// filter := model.ReviewFilterForQuery{
	// 	UserID: userID,
	// }

	// total, _ := GetReviewCount(ctx, filter)
	// 过滤非匿名点评

	// 获取用户收到的赞数、被打赏积分数、关注的课程数

	return &user.UserActivity{}, nil
}

func GetUserByIDs(ctx context.Context, userIDs []int64) (map[int64]user.UserMinimal, error) {
	result := make(map[int64]user.UserMinimal)
	if len(userIDs) == 0 {
		return result, nil
	}
	u := repository.Q.UserPO
	userPOs, err := u.WithContext(ctx).Where(u.ID.In(userIDs...)).Find()
	if err != nil {
		return result, err
	}
	for _, userPO := range userPOs {
		user := converter.ConvertUserMinimalFromPO(userPO)
		result[user.ID] = user
	}
	return result, nil
}

// 共用函数，用于获取用户基本信息和详细资料并组装成domain.User
func GetUserDetailByID(ctx context.Context, userID int64) (*user.UserDetail, error) {
	u := repository.Q.UserPO
	userPO, err := u.WithContext(ctx).Where(u.ID.Eq(userID)).Take()
	if err != nil {
		return nil, err
	}
	user := converter.ConvertUserDetailFromPO(userPO)
	return &user, nil
}

func buildUserDBOptionFromFilter(ctx context.Context, q *repository.Query, filter user.UserFilterForQuery) repository.IUserPODo {
	builder := q.UserPO.WithContext(ctx)
	u := q.UserPO
	if filter.Page > 0 || filter.PageSize > 0 {
		builder = builder.Offset(int(util.CalcOffset(filter.Page, filter.PageSize))).Limit(int(filter.PageSize))
	}
	if filter.Order != "" {
		field, ok := u.GetFieldByName(filter.Order)
		if ok {
			if filter.Ascending {
				builder = builder.Order(field)
			} else {
				builder = builder.Order(field.Desc())
			}
		}
	}
	return builder
}

func GetUserList(ctx context.Context, filter user.UserFilterForQuery) ([]user.UserMinimal, error) {
	q := buildUserDBOptionFromFilter(ctx, repository.Q, filter)
	userPOs, err := q.Find()
	if err != nil {
		return nil, err
	}

	result := make([]user.UserMinimal, 0)
	for _, userPO := range userPOs {
		result = append(result, converter.ConvertUserMinimalFromPO(userPO))
	}
	return result, nil
}

func GetUserCount(ctx context.Context, filter user.UserFilterForQuery) (int64, error) {
	filter.Page, filter.PageSize = 0, 0
	q := buildUserDBOptionFromFilter(ctx, repository.Q, filter)
	return q.Count()
}

func UpdateUserProfileByID(ctx context.Context, userProfileDTO dto.UserProfileDTO, userID int64) error {
	u := repository.Q.UserPO
	newUserPO := converter.ConvertUserProfileToPO(userProfileDTO)
	newUserPO.ID = userID
	err := u.WithContext(ctx).Save(&newUserPO)
	if err != nil {
		return err
	}
	return nil
}
